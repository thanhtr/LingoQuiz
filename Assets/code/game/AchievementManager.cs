...
public class AchievementManager : MonoBehaviour
{
    ...
    public string AchievementToJson(Achievement[] achievements)
    {
        string json_achievement = "";
        for (int i = 0; i < achievements.Length; i++)
        {
            json_achievement += JsonMapper.ToJson(achievements[i]);
            if (i != achievements.Length - 1)
                json_achievement += "|";
        }
        return json_achievement;
    }

    public Achievement JsonToAchievement(string json_string)
    {
        Achievement achievement = JsonMapper.ToObject<Achievement>(json_string);
        return achievement;
    }
}
